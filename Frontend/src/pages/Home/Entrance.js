...
function Entrance(props) {
  useEffect(() => {
    ...
    for (var i = 0; i < numMetaballs; i++) {
      ...
      metaballs.push({
        x: Math.random() * (width - 2 * radius) + radius, // 球在X軸的位置
        y: Math.random() * (height - 2 * radius) + radius, // 球在Y軸的位置
        vx: (Math.random() - 0.5) * 3, // 球X軸移動的速度
        vy: (Math.random() - 0.5) * 3, // 球Y軸移動的速度
        r: radius * 0.75, // 球的半徑
      })
    }
      ...
      <div className="bg-background">
        <section id="canvas1">
          <div className="encontainer">
            {/* <span className="title">Artilize</span> */}
